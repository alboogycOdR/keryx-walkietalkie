---
plan_version: 2.2
last_updated: 2026-08-18T19:16:06Z
overall_status: in_progress
orchestrator_notes: "Plan v1.0 — 26 tasks from 3 specs. Territory: pubspec.yaml/analysis_options.yaml/.github/**/.gitignore/README.md, lib/core/theme/**, android/**+lib/services/discovery/**+test/services/discovery/**, lib/core/floor/**+test/core/floor/** all FROZEN. android/** serialized 001→019→026. lib/core/audio/** serialized 010→011. Dossiers in dossiers/TASK-NNN.md. S5/S5B INACTIVE. Full review/incident history in git log / REVIEW.md. Known unresolved risk: plan_commit doesn't re-diff before commit — lost-update race hit THREE times now (TASK-004, TASK-008, TASK-022 claim/in_progress transitions swept into ORCH commits), always content-safe but a real pattern, not mechanically fixed. SPEC/PLAN FOLLOW-UPS OWED (index only — full detail in each task's Review_Findings + REVIEW.md): (a) font OFL/LICENSE; (b)+(c) .github/** CI task; (d) BLOCKS TASK-007 (blocked/fenced) — roomId base32, RECOMMEND RFC 4648 matching token-svc; (e) token-svc X-Forwarded-For; (f) R4 per-room caps; (g) keryx-evt.v1 vs FR-044; (h) tune_burst contradiction; (i) KRX-024 60ms marker; (j) SFX ducking driver; (k) BLOCKS TASK-013/014 (blocked/fenced; 012 clean) — wrap-vs-clamp, RECOMMEND clamp; (l) LINK_DEGRADED/LOCAL_FALLBACK undefined; (m) MANDATORY lib/core/state/** successor task before TASK-017 — NOT CREATED, now also owed to TASK-022 (EMG/station-count/active-speaker routed around the frozen reducer, needs reconciling); (n) peerId base32, rests on TASK-009; (o) protocol ts/quality/seq types; (p) BLOCKS TASK-016 — settle curve, RECOMMEND amend DS §4 to PT's value; (q) MANDATORY lib/core/theme/** successor task before 012-017 fully integrate — NOT CREATED; (r) DS §3 vs PT telltale font; (s) BLOCKS TASK-020 — LOCAL discovery wire contract spec-silent; (t) §8.6 TX_GRANT has no sender field, receiver can't authenticate a grant — matters for TASK-023 byzantine cases. No git remote — CI has never run. NEW SPEC uncommitted in specs/: KERYX_World_Band_Radio_Spec_v1.0.md (Phase 2, not now). PROGRESS: TASK-001-006/008/009/010/019/022 all done and merged. GB 7/7 first-pass. TASK-022 unlocked TASK-023 (soak harness, TBD, needs owner) — gets a head start, VirtualClock+LoopbackHub already built by TASK-022. DISPATCH (2026-08-18T15:15Z): dispatched GB (self-selects from {009,011}, transcript GB-20260818-151341.log) and CX (self-selects from {012,015}, transcript CX-20260818-151351.log) — both clean, no fenced tasks in either pool. TBD tasks still unowned: TASK-016, TASK-018 (held back deliberately — needs a dim-mode setting TASK-008 didn't persist), TASK-023, TASK-026. STATUS SCAN (2026-08-18T15:20Z): CX claimed TASK-012 (display glass), territory clean so far, no code commits yet — freshly started, healthy. GB hasn't landed a claim commit yet either but its process is confirmed alive (started same moment as dispatch) — too early to call stale, not drift. No blocked-task triage action beyond standing recommendations. STATUS SCAN (2026-08-18T15:25Z): TASK-009 (GB, identity) landed needs_review — territory clean (14 files, lib/core/identity+test/core/identity only), commit 49eae05 tagged. GB proactively pinned RFC 4648 for peerId (matching follow-up (n)'s recommendation) without being told — good sign, verify at review. TASK-012 (CX, display glass) also needs_review — territory clean (3 files), commit 6f08e5f tagged. Both builders idle. No blocked-task triage action beyond standing recommendations. REVIEW (2026-08-18T18:14Z): TASK-009 (GB, identity) APPROVED first-pass, merged 272a85c — GB now 7/7. Territory clean, preflight genuine, 132/132 post-merge on master. RFC 4648 verified byte-exact against an independent implementation (not taken from GB's comment): alphabet is the RFC 4648 §6 set lowercased index-for-index, both frozen vectors recomputed and matched, plus 10 ORCH vectors covering every 5-bit tail residue. NOTE the dispatch-time belief that GB pinned RFC 4648 unprompted is WRONG — dossiers/TASK-009.md L12 instructed exactly that, so the lowercase choice is ORCH's own. FOLLOW-UP (n) RESOLVED for peerId (alphabet confirmed) but spawns (n2): peerId is lowercase while token-svc roomId, TASK-006's golden peer and TASK-022's merged floor fixtures (test/core/floor/floor_engine_test.dart:6, AAA2222222 etc.) are all uppercase — harmless now (one Dart derivation, arbiter treats peerId as opaque) but election is a lexicographic minimum and ASCII sorts all lowercase after all uppercase, so a mixed population elects strictly by case; §8.6 must pin alphabet AND case per identifier before iOS Phase 3. (n) also gives the precedent to RULE (d) and unblock TASK-007: token-svc already implements roomId as RFC 4648 uppercase unpadded — ratify that and clear TASK-007's blocked/SPEC_AMBIGUITY. Two identity findings are owed to TASK-020: displayNames takes a raw String so a remote cs of 'BRAVO-7 (2)' makes two peers render identically (ORCH-reproduced), and it throws ArgumentError on duplicate/empty peerIds fed from the wire — sanitize remote cs through Callsign.parse before calling. lib/core/identity/** now FROZEN. UNLOCKS TASK-020 (only dependent; TASK-006+TASK-009 both done) — fully eligible, still TBD, needs an owner. REVIEW (2026-08-18T19:16Z): TASK-012 (CX, display glass) APPROVED first-pass, merged 98484a8 — CX now 4/6 first-pass. Territory clean (3 files), c8b9872 preflight PRESENT (NO second consecutive skip — rework trigger did not fire; TASK-005 was the one-off lapse, 008+012 both carry it), PLAN.md discipline clean, all 7 ACs verified against spec text, full suite 137/137 post-merge on master. Theme headline check PASSES: amber/signal/substrate/ghost/typography all via KeryxTheme; telltale row uses KeryxTheme.telltale = Barlow Condensed (token-referenced, not hardcoded). REAL Criterion-5 finding recorded (not reworked, unfixable in CX territory): glass-frame material hardcoded — Color(0xFF0A0F0C) border + two Color.fromRGBO shadows + Colors.transparent — these are PT .glass L51-58 inline chrome that PT never abstracts into :root, so the frozen theme exposes NO token for them. This is the PREDICTED consequence of dispatching 012 before the un-created theme-successor task (q). ACTION — (q) is now CONCRETE and HIGHER PRIORITY: it must add glass-recess material tokens (border/inner-shadow/highlight) and TASK-012 must be refactored to consume them BEFORE TASK-017 integrates the glass; TASK-013/014/015/016 will hardcode the same PT inline chrome until (q) lands. (r) DS §3-vs-PT telltale font is now LIVE/RENDERED (Barlow Condensed on the face) — settle DS §3 before more glass work. TASK-017 stays BLOCKED (Depends_On 004+012+013+014+015+016: 004 done, 012 now done, 013 blocked, 014 blocked, 015 pending, 016 pending) — NOTHING new unlocks from this merge. lib/features/display/** now FROZEN. Next: create theme-successor task (q) with the concrete glass-recess token list; still owed — (m) state successor, identity successor for the 10 findings, rule on (d)/(k)/(p)/(n2), assign TBD owners (016, 018, 020, 023, 026)."
---
