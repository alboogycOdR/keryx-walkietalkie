---
plan_version: 1.3
last_updated: 2026-08-18T10:16:44Z
overall_status: in_progress
orchestrator_notes: "Plan v1.0 — first real decomposition from 3 specs (KERYX_Product_Technical_Spec_v1.1.md, KERYX_UI_Design_Specification_v1.0.md, keryx-face-prototype.html). 26 tasks. Territory rules: pubspec.yaml/analysis_options.yaml/.github/**/.gitignore/README.md are FROZEN after TASK-001. android/** is a serialized chain (001 → 019 → 026). lib/core/audio/** is a serialized chain (010 → 011). Dossiers exist for every task in dossiers/TASK-NNN.md. S5/S5B are defined but INACTIVE for this project — never assign them. History of TASK-002's review, the two tooling-bug fixes (dispatch.ps1/worktree.ps1 branch-name, plan_commit repo-root), and the PLAN.md lost-update incident lives in git log / REVIEW.md — not repeated here every scan; see commits around 196218e/b8171fe/c66becf/e18e820 if revisiting. Known unresolved risk: plan_commit does not re-diff immediately before its commit, so the lost-update race (last seen c66becf) can recur under concurrent dispatch — no fix applied yet, flagged for a future hardening task. STATUS SCAN (2026-08-18T10:05Z): TASK-001 (CX, Flutter scaffold) is needs_review — territory clean (37 files, all inside its Owned_Paths incl. lib/**), single commit `789d30a` tagged [TASK-001]. Note: this scaffold's lib/main.dart is flutter-create boilerplate; TASK-017 owns lib/main.dart exclusively per its Owned_Paths and will replace it wholesale — expected overlap, not a violation (TASK-001's own Owned_Paths includes lib/** too), flag to the TASK-017 reviewer as context, not a rework trigger now. TASK-002 done (merged e18e820). TASK-003 (GB, token-svc/**) in_progress, actively building (FastAPI app + tests + Dockerfile present in worktree, uncommitted, correctly gitignoring its own .venv/), no commits yet — too early to call stale. No blocked tasks. REVIEW (2026-08-18T10:16Z): TASK-001 APPROVED first-pass and merged as 2c40de2 — branch task/TASK-001-cx deleted, CX worktree detached onto master and ready for reuse. The FOUNDATION has landed: pubspec.yaml/pubspec.lock/analysis_options.yaml/.metadata/.gitignore/README.md/.github/** are now FROZEN — any change to them needs a dedicated ORCH-created integration task, no builder may touch them. NEWLY ELIGIBLE (all 8 dependents of TASK-001, each with no other dependency): TASK-004 (CX), TASK-005 (CX), TASK-006 (GB), TASK-007 (GB), TASK-008 (CX), TASK-009 (GB), TASK-010 (GB), TASK-019 (GB) — all pending, all dispatchable now. Note TASK-005 is the gate for TASK-012/013/014/015/016 and TASK-010 gates TASK-011, so prioritise 005 and 010 in the next wave. Correction to the previous scan: TASK-001's lib/main.dart is NOT flutter-create boilerplate — CX replaced the counter demo with a 16-line ProviderScope boot shell whose doc comment explicitly hands off to TASK-017, so there is nothing for TASK-017 to fight. THREE FOLLOW-UPS OWED (from TASK-001 review, none blocking, all need ORCH task creation — full detail in TASK-001 Review_Findings and REVIEW.md): (a) font OFL/LICENSE files are missing from assets/fonts/ — licence-compliance defect, MUST land before any release build; assets/fonts/** is unfrozen so a small task can own it; (b) CI needs an explicit flutter-version pin (currently floats on channel:stable away from the locally-pinned 3.41.6); (c) CI needs --coverage + threshold to make §9 NFR-10 enforceable — sequence after TASK-004 lands the reducer. (b) and (c) share .github/** so they should be ONE integration task. Also unresolved: no git remote is configured, so the CI workflow has never actually run — its correctness is static-inspection only. NEW SPEC ARRIVED (uncommitted at review time): specs/KERYX_World_Band_Radio_Spec_v1.0.md (World Band / WRLD, receive-only, explicitly NOT Phase 1) plus a specs/README.md edit — needs a /devteam-decompose pass before any of it becomes tasks; deliberately left uncommitted by this review commit. TASK-003 (GB, token-svc/**) went needs_review at 840ce0c — review it next. Next: /devteam-review TASK-003, then /devteam-dispatch the 8 unlocked tasks (005 and 010 first)."
---
