---
plan_version: 1.9
last_updated: 2026-08-18T14:05:15Z
overall_status: in_progress
orchestrator_notes: "Plan v1.0 — 26 tasks from 3 specs. Territory: pubspec.yaml/analysis_options.yaml/.github/**/.gitignore/README.md FROZEN since 2c40de2. android/** serialized 001→019→026 (019 now at needs_review). lib/core/audio/** serialized 010→011. Dossiers in dossiers/TASK-NNN.md. S5/S5B INACTIVE — never assign. Full review/incident history in git log / REVIEW.md, not repeated here. Known unresolved risk: plan_commit doesn't re-diff before commit — real lost-update race hit twice, self-healed both times, not mechanically fixed. SPEC/PLAN FOLLOW-UPS OWED (full detail in each task's Review_Findings + REVIEW.md — index only): (a) font OFL/LICENSE missing; (b)+(c) .github/** CI pin+coverage task; (d) BLOCKS TASK-007 — roomId base32 alphabet undefined; (e) token-svc X-Forwarded-For trust; (f) R4 per-room caps unbuilt; (g) keryx-evt.v1 vs FR-044 prose; (h) tune_burst ducking direction contradiction; (i) KRX-024 60ms marker unspecified; (j) SFX ducking release has no driver; (k) BLOCKS TASK-012/013/014 — channel/code wrap-vs-clamp undefined; (l) LINK_DEGRADED/LOCAL_FALLBACK undefined; (m) MANDATORY BLOCKER for TASK-017 (TASK-022 hits it at integration, not dispatch) — lib/core/state/** frozen with no owner, RadioState missing required fields, ORCH must create successor task (010→011 pattern) — STILL NOT DONE; (n) peerId same base32 gap as roomId, TASK-009 must pin RFC 4648 — TASK-019 checked against this and is CLEAN: it never derives peerId, takes it as an opaque required String in `DiscoveryConfig`, and the only base32 token in its whole diff is a comment cross-referencing §8.7/TASK-007; its `ch` prefix is hex, so no alphabet is pre-empted. (n) still rests entirely on TASK-009; (o) protocol ts/quality/seq types normative-by-implementation, write into §8.6; (p) BLOCKS TASK-016 — DS §4 says `settle` is "320 ms, ease-out" but PT `:root` L16 says `cubic-bezier(.16,1,.3,1)`; TASK-005 shipped PT's value (correctly, per its own "match PT exactly" instruction), so DS §4 must be amended before TASK-016 dispatches or its criterion quoting "ease-out" fails a correct builder; (q) BLOCKS TASK-012/013/014/015/016/017 — `lib/core/theme/**` frozen at merge without DS §4's 1 dp key-travel constant, the 2-3% noise-overlay token, the 20%-gradient constant, an Inter 600 style, or any faceplate seam (DS §8/FR-101/KRX-018/KRX-096 all need runtime-swappable hue/material, but tokens are flat `static const`); ORCH must create a `lib/core/theme/**` successor task on the 010→011 pattern BEFORE the five unlocked face tasks dispatch, or six territories hardcode what the theme never exposed; (r) DS §3's role table puts telltales under Share Tech Mono while PT `.tell` uses Barlow Condensed — TASK-005 followed PT; settle in §3 before TASK-012; (s) BLOCKS TASK-020/TASK-026 INTEROP — the entire LOCAL discovery wire contract is spec-silent and is now normative-by-implementation from TASK-019: the spec never defines the **channel-hash prefix** construction (algorithm, length, encoding, whether region salts it) though FR-041/§8.3 both name it, so GB pinned `hex(SHA-256(utf8("region|ch|code")))[:8]` and disclosed it in its Progress_Note, README and dartdoc — deliberately NOT §8.7's `b32(HMAC-SHA256("KERYX.v1", ...))[:16]`, with the facade taking an opaque prefix so a `roomId` slice can replace it without a wire fork; the spec also never defines the UDP beacon port (48721), payload (`{v,t,peer,cs,ch,p}`) or broadcast address; and §8.3 step 2 requires the signaling port "advertised in NSD" while FR-041 enumerates a three-key TXT set (`cs`/`ch`/`v`) with no port key, so the `p` record ORCH's own dossier instructed is spec-unbacked. Write all of this into §8.1/§8.3 BEFORE TASK-020 dispatches or two implementations will not match. Related, non-defect: an 8-hex (32-bit) unkeyed hash over ch 1-99 × code 00-38 × region is trivially reversible by an on-LAN observer, so `ch` is not confidentiality — but TS L361 already concedes exactly this ("on pure LOCAL, privacy codes are client-side filters only — like real CTCSS") and L511 assigns regional salts as the drive-by-scanning mitigation, which GB's construction does include; literal and threat-model compliance both hold. No git remote — CI has never run. NEW SPEC uncommitted in specs/: KERYX_World_Band_Radio_Spec_v1.0.md (Phase 2, not now). STATUS SCAN (2026-08-18T13:45Z): CX self-selected TASK-005 (theme tokens) — needs_review, territory clean (2 files, lib/core/theme+test/core/theme only), commit f680093 tagged. GB self-selected TASK-019 (NSD discovery, not TASK-022 — its own choice between two eligible high-priority options) — needs_review, territory clean (18 files, android/**+lib/services/discovery/**+test/services/discovery/**), commit 7a1f9d4 tagged. TASK-022 still pending/assigned-GB, unclaimed — GB picked 019 first. Both builders idle again. No blocked tasks, no critical findings, no drift. REVIEW 2026-08-18T13:56Z: TASK-005 APPROVED first-pass and merged as ef62ee8 (all 10 colour hexes exact vs PT :root; the single divergence, settle curve, is PT-correct and DS-prose-wrong — see (p)). lib/core/theme/** now FROZEN. **BIGGEST UNLOCK SO FAR — TASK-012/013/014/015/016 are all now fully eligible** (each Depends_On TASK-005 only; 012/013/014/015 already assigned CX, 016 still TBD). TASK-018 stays blocked (also needs TASK-008, pending). REVIEW 2026-08-18T14:05Z: TASK-019 APPROVED first-pass and merged as b229010 — GB now 5/5. Territory clean (18 files), preflight re-run by ORCH and provably genuine, 3 PLAN.md commits all inside its own block, 68/68 reproduced at 7a1f9d4 and 71/71 post-merge on master. android/** serialization chain 001→019→026 held: manifest is a pure 11-line prepend, MainActivity gained only a configureFlutterEngine override, nothing in TASK-001's scaffold restructured. **android/**, lib/services/discovery/** and test/services/discovery/** now FROZEN.** **UNLOCKS TASK-026** (foreground service, KRX-080) — TASK-019 was its ONLY Depends_On, so it is now fully eligible; still Assigned_To: TBD and needs an owner. Caveat for whoever takes it: TASK-026 inherits android/** and owes the 7 native findings in TASK-019's Review_Findings, above all the MulticastLock leak on async NSD failure (battery drain) and the unserialized resolveService (drops peers, threatens NFR-04). Honest gap: NsdPlugin.kt has never been compiled — no JVM test tree, no JDK/Gradle wrapper here, and CI has never run for want of a remote. Next: still owed BEFORE dispatching the five face tasks — (q) theme successor task, (p) DS §4 settle amendment, (k) wrap-vs-clamp, and (m) the lib/core/state/** successor task; and (s) before TASK-020."
---
