---
plan_version: 1.5
last_updated: 2026-08-18T12:15:00Z
overall_status: in_progress
orchestrator_notes: "Plan v1.0 — 26 tasks from 3 specs. Territory: pubspec.yaml/analysis_options.yaml/.github/**/.gitignore/README.md FROZEN since 2c40de2. android/** serialized 001→019→026. lib/core/audio/** serialized 010→011. Dossiers in dossiers/TASK-NNN.md. S5/S5B INACTIVE — never assign. Full review/incident history in git log / REVIEW.md, not repeated here. Known unresolved risk: plan_commit doesn't re-diff before commit — a real lost-update race has hit twice (c66becf, and TASK-004's handoff), both self-healed by builder discipline, not mechanically fixed. SPEC/PLAN FOLLOW-UPS OWED (full detail in each task's Review_Findings + REVIEW.md — this is an index, not the content): (a) font OFL/LICENSE missing; (b)+(c) .github/** CI pin+coverage task (after TASK-004 lands); (d) BLOCKS TASK-007 — roomId base32 alphabet undefined in spec, token-svc pinned RFC 4648 by implementation; (e) token-svc trusts X-Forwarded-For unconditionally, relay task must restrict ingress; (f) R4 per-room caps unbuilt; (g) keryx-evt.v1 contract vs FR-044 prose; (h) tune_burst ducking direction contradicts itself across §7.1/FR-006 vs §7.2 (GB built to §7.2); (i) KRX-024 60ms end-of-TX marker has no spec id or code; (j) SFX ducking release has no driver wired yet; (k) BLOCKS TASK-012/013/014 — channel/code wrap-vs-clamp undefined in spec, reducer only exposes absolute TuneTo, decide before those three dispatch; (l) LINK_DEGRADED/LOCAL_FALLBACK undefined in §8.2, TASK-004 pinned LOCAL_FALLBACK≡(idle,local) by implementation; (m) BLOCKS TASK-017+TASK-022 — RadioState has no flags/STN-count/active-speaker that DS §6's State Catalogue requires, and neither dependent task owns lib/core/state/** to add them — resolve during TASK-004's rework or ORCH must create a successor task. No git remote — CI has never run. NEW SPEC uncommitted in specs/: KERYX_World_Band_Radio_Spec_v1.0.md (Phase 2, not now). REVIEW (2026-08-18T12:08Z): TASK-004 (CX) REWORK, first rework of the project (CX now 1/2 first-pass) — NOT merged, branch retained, back to in_progress. 5/6 criteria pass; criterion 6 (100% branch coverage) failed on a fake metric: the toolchain's --branch-coverage LCOV had no BRF/BRH denominator and its BRDA records structurally cannot represent an untaken branch — proven by mutation testing (deleted 3 phase guards, full suite stayed green). Fix is small: negative tests for the 3 guards + a table-driven transition matrix, and stop quoting a percentage this toolchain doesn't compute. LESSON FOR ALL UNITS, worth keeping visible: a coverage percentage is only evidence if the tool actually measures the thing named. DISPATCH (2026-08-18T12:15Z): GB launched (transcript .devteam/launch/GB-20260818-121153.log) against one of {006,009,019} (self-selects; not 007, blocked by (d)) — its worktree was stale-but-safe (detached at a0cc2c5, a real ancestor of master 9 commits back, none touching GB's territories, so no refresh needed). CX launched to resume the TASK-004 rework (transcript .devteam/launch/CX-20260818-125045.log) — its worktree was already correctly on task/TASK-004-cx, resume path, nothing to refresh. Next: /devteam-status once both have had time to work."
---
