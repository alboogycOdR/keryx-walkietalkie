---
plan_version: 1.1
last_updated: 2026-08-18T09:31:00Z
overall_status: in_progress
orchestrator_notes: "Plan v1.0 — first real decomposition from 3 specs (KERYX_Product_Technical_Spec_v1.1.md, KERYX_UI_Design_Specification_v1.0.md, keryx-face-prototype.html). 26 tasks. FIRST DISPATCH WAVE: TASK-001 (CX — Flutter scaffold + CI, creates lib/test/android/assets territory) and TASK-002 (GB — relay/ Docker compose). TASK-003 (GB, token-svc/) has no dependencies and is GB's queue-next after TASK-002. After TASK-001 merges, second wave fans out: CX → TASK-004 (state machine) then TASK-005 (theme tokens); GB → TASK-006/007/009/010 in any order. Territory rules: pubspec.yaml/analysis_options.yaml/.github/**/.gitignore/README.md are FROZEN after TASK-001 — all Phase-1 dependencies and font declarations are pre-loaded there; any later pubspec change requires a new ORCH-owned integration task, never a drive-by edit. android/** is a serialized chain (001 → 019 → 026). lib/core/audio/** is a serialized chain (010 → 011). Dossiers exist for every task in dossiers/TASK-NNN.md — builders read theirs FIRST on claim. S5/S5B are defined but INACTIVE for this project (autopilot.json builders.active = GB, CX) — never assign them. DISPATCH WAVE 1 (2026-08-18T09:30Z): GB launched against relay/-territory work (TASK-002 next-eligible, no deps) and CX launched against TASK-001 (Flutter scaffold) — both headless, detached windows, transcripts in .devteam/launch/GB-20260818-093034.log and CX-20260818-093036.log. Pre-dispatch: fixed two real bugs in scripts/dispatch.ps1 and scripts/worktree.ps1 that hardcoded the branch name 'main' instead of reading autopilot.json git.base_branch — this repo's integration branch is 'master' (commit 196218e). Worktrees: wt-grok-walkietalkie-keryx, wt-codex-walkietalkie-keryx, both detached at master tip before launch. Run /devteam-status to check progress."
---
