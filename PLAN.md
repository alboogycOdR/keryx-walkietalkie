---
plan_version: 2.5
last_updated: 2026-08-19T16:06:04Z
overall_status: in_progress
orchestrator_notes: "Plan v1.0 — 28 tasks from 3 specs. FROZEN territories (merged): pubspec/analysis_options/.github/.gitignore/README, lib/core/theme, android+lib/services/discovery, lib/core/floor, lib/core/state, lib/core/settings, lib/core/protocol, lib/core/audio(010+011, chain COMPLETE), lib/core/identity, lib/features/display. Serial chains: android 001→019→026, audio 010→011 (DONE). Dossiers in dossiers/. S5/S5B INACTIVE. Full review/incident detail in git log + REVIEW.md + each task's Review_Findings — NOT duplicated here. Model config: all ORCH judgment + review now on claude-opus-5 (Fable retired 2026-08-18, commit 426d9a9). Known risk: plan_commit doesn't re-diff before commit — lost-update race hit 3x (all content-safe, not mechanically fixed). FOLLOW-UPS OWED (index; RECOMMEND = my proposed ruling, not yet applied): (a) font OFL/LICENSE; (b)+(c) .github CI task; (d) BLOCKS TASK-007(fenced) roomId base32 — RECOMMEND RFC 4648 uppercase, token-svc precedent; (e) token-svc X-Forwarded-For; (f) R4 per-room caps; (g) keryx-evt.v1 vs FR-044; (h) tune_burst contradiction; (i) KRX-024 60ms marker; (j) SFX ducking driver; (k) BLOCKS TASK-013/014(fenced) wrap-vs-clamp — RECOMMEND clamp; (l) LINK_DEGRADED/LOCAL_FALLBACK undefined; (m) MANDATORY lib/core/state successor task before TASK-017 (RadioState missing EMG/station-count/active-speaker; TASK-022 routes them around the reducer) — CREATED as TASK-027; (n) peerId base32 RESOLVED (RFC4648 lowercase); (n2) peerId lowercase vs roomId/floor-fixtures uppercase — election is lexicographic min so case decides a mixed population; pin case per identifier before iOS; (o) protocol ts/quality/seq types; (p) BLOCKS TASK-016 settle curve — RECOMMEND amend DS §4 to PT value; (q) MANDATORY lib/core/theme successor task (glass-recess material tokens: border/inner-shadow/highlight) — CREATED as TASK-028, see below; (r) DS §3 vs PT telltale font, now live-rendered; (s) BLOCKS TASK-020 LOCAL discovery wire contract spec-silent; (t) §8.6 TX_GRANT no sender field; identity findings owed to TASK-020 (sanitize remote cs, dup/empty peerId throws). No git remote — CI never run. NEW SPEC uncommitted: specs/KERYX_World_Band_Radio_Spec_v1.0.md (Phase 2). PROGRESS: 13 done (001-006,008-012,019,022 — the old '11 done' count was off by one). GB 8/8 first-pass; CX 4/6 (1 rework=004r1). STATUS SCAN (2026-08-19T00:00Z): both builders idle, no active branches, no drift. Only blocked=007/013/014 (all fenced on spec ambiguity, awaiting ORCH ruling per (d)/(k)). ORCH ACCOUNTABILITY: dispatching TASK-012 while (q) was flagged-mandatory-but-uncreated was a real sequencing gap — TASK-012 correctly hardcoded PT's un-tokenized glass chrome (unfixable in its territory) and now lib/features/display is FROZEN with that debt baked in. CONSEQUENCE FOR NEXT DISPATCH: TASK-015 (PTT/keys, CX) and TASK-016 (grille, TBD) are technically eligible+unfenced, but both consume the SAME glass/key material the theme doesn't expose, so dispatching them now replicates TASK-012's hardcoded-chrome debt into two more soon-frozen territories. 'Dispatch what's clean' has hit its limit for the face wave: clean-by-deps is no longer clean-by-consequence until (q) lands. Genuinely still-clean work with NO theme-token dependency: TASK-011 (GB, character DSP, extends audio) and TASK-020 (LAN signaling, TBD — needs owner, inherits identity findings). Next: recommend creating (m)+(q) successor tasks and ruling (d)/(k)/(p) before more face widgets; can still dispatch TASK-011 cleanly meanwhile. STATUS SCAN (2026-08-19T05:40Z): (m)/(q) DONE — TASK-027 (state successor, bumped to CRITICAL as face-wave critical path) + TASK-028 (theme successor) created, plan now 28 tasks. TASK-011 (GB, character DSP) landed needs_review — territory clean (19 files all lib/core/audio+test+assets/sfx, incl. expected edits to TASK-010's files per the 010→011 chain), commit 8630464 tagged. LOST-UPDATE RACE #4 (prevented, not damaging): CX was dispatched for TASK-027 but hit GB's TASK-011 handoff momentarily-uncommitted in the shared PLAN.md; since plan_commit writes the whole file, CX correctly refused to claim (would have swept GB's handoff into its claim), removed its own edits, and exited with TASK-027 still pending — same disciplined self-abort GB/CX have shown before. No corruption; state resolved (TASK-011 committed at needs_review, PLAN.md clean). Re-dispatching CX for TASK-027 now that the window is single-writer. This is now the 4th occurrence of the plan_commit race — the mechanical fix (re-diff immediately before commit, or per-task-block commit) is overdue. GB idle after TASK-011. REVIEW (2026-08-19T10:26:59Z): **TASK-011 APPROVED first-pass, merged 0c526b4** — GB now 8/8. Territory clean (19 files, exclusion diff empty); all 4 ACs verified against TS §7.2 L260 / FR-061 L145 text; full suite independently reproduced 159/159 + analyze clean from a clean `git archive` extraction; TASK-010's audio tests all still green (sfx_engine 11, manifest_and_assets 5, bed_mixer 7) and all 22 WAVs re-verified bit-identical to the committed generator's output. UNLOCKS NOTHING (no task has Depends_On: TASK-011); `lib/core/audio/**` is now FROZEN with the 010→011 chain complete. New follow-ups owed, all NON-BLOCKING and recorded in TASK-011's Review_Findings: (u) FR-061 has no end-to-end wire — `SfxEngine.applySquelch`/`VoiceRxProcessor.squelchDetent` have zero consumers outside the (now frozen) audio territory, so the settings→audio glue needs its own task before the knob 'actually functions'; (v) TS §7.2 is silent on knee width, gate dB endpoints, compressor threshold/envelope times and the Light-vs-Full scaling curve — GB's values (knee 12/8/4, gate −12…−40 dBFS, thr −18 dBFS, 5/50 ms, hiss ×0/0.5/1.0) are now normative-by-implementation and should be lifted into the spec. GB heuristic recurrence: the shipped default intensity (Light) is the least-tested one — same 'tests one representative arrangement' pattern as TASK-022. ORCH cleanup note: GB's worktree was deregistered and the branch deleted, but the now-empty dir C:\\CLAUDECODE_TOOLSETS\\wt-grok-walkietalkie-keryx is held open by some process and could not be rmdir'd — clear it before the next GB dispatch. GB and CX both idle. Next: /devteam-review TASK-027 (CX, lib/core/state, needs_review since 2026-08-19T05:40Z) — untouched by this review; then rule (d)/(k)/(p) and dispatch TASK-028 (theme successor) before any more face widgets. REVIEW (2026-08-19T16:06:04Z): **TASK-027 REWORK — criterion 9 only, NOT merged, branch task/TASK-027-cx retained, Status -> in_progress.** CX 4/7 first-pass, 2 rework (004 resolved, 027 open). Eight of nine criteria pass; this is CX's best implementation of the project. **BRIDGE DESIGN (A) IS RATIFIED AS BUILT** — CX built (A) not (B), and one-wayness is proven STRUCTURALLY: radio_state.dart has ZERO imports, so the reducer cannot reference the bridge/engine/Riverpod even in principle. Station count correctly sourced from the presence feed via bridge.updateRoster (frozen engine still has no _peers getter). Territory clean (5 files, exclusion diff empty, eb585a6); merges CLEAN into master; PLAN.md discipline flawless; **c8b9872 preflight PASTED VERBATIM for the first time — that standing CX nit is CLOSED**; Test_Evidence honest (LF 167/LH 145, no percentage claimed — the TASK-004 rework defect was NOT repeated); tuneDelta correctly ABSENT with deferral stated, so unruled (k) was not silently decided. **TASK-004 REGRESSION: NONE** — all 11 original tests green, matrix EXTENDED 35 legal/77 illegal over 8x14 (112 pairs) -> 123 legal/85 illegal over 8x26 (208 pairs) against an independent oracle, and ORCH mutation-proved it has teeth. Independent full suite on the MERGED tree: 162/162 + analyze clean. **REWORK CAUSE:** ORCH deleted both NEW guards (RosterUpdated stationCount>=0 and SignalQualityUpdated range) and the suite still passed 16/16 — provably untested false-branches under NFR-10 100% branch, the identical mutation that condemned TASK-004 r1. Diagnosis is now exact: **CX tests guards it INHERITS and omits tests for guards it AUTHORS**; give that instruction at DISPATCH. Fix is ~6 lines of test + a parameter rename (ActiveSpeakerChanged.callsign actually carries a peerId and would mislead TASK-017). NEW FOLLOW-UPS OWED, all ORCH-owned and NON-BLOCKING, recorded in TASK-027 Review_Findings: (w) the bridge drops TotWarn/TotCut so DS §6's named face state "TX time-out warning" (plus "TX denied/busy" and "LAN?") still has NO RadioState field — TASK-022 already emits the effect, so TASK-017 would have to read the floor stream directly, i.e. the exact two-source rendering (A) exists to prevent; this is ORCH sequencing debt, not a CX miss (criterion 1 enumerated 7 telltales and CX delivered all 7), and needs its own follow-on BEFORE TASK-017; (x) FR-069's aggregate rule has two sub-cases (TX: that station's link; idle: worst active peer) and nothing records that selecting between them is the telemetry feed's job — pin before KRX-035; (y) FloorIdleSettled dropped, so the SetMode gate is phase-approximate rather than §8.4's debounced floor-idle; (z) PowerOff leaves isEmergency/stationCount/MON/SCAN/VOX/PRV/arbiterId/signalQuality set, so a powered-off radio can project EMG lit and STN 4 — TASK-017 must not render telltales in RadioPhase.off; (aa) engine roster and RadioState.stationCount are fed independently and can diverge; (bb) signalQuality defaults to S1 with no nullable "unknown" (§8.9 spec-silent); (cc) the test oracle's `_ => state` wildcard would silently absorb a future sealed event. **TASK-017 IS NOT UNBLOCKED BY THIS.** Its Depends_On is TASK-004, 012, 013, 014, 015, 016 — it does NOT list TASK-027. 004 and 012 are done; 013/014 remain Status:blocked on SPEC_AMBIGUITY awaiting ruling (k); 015/016 remain pending. TASK-027 removes the *state* obstacle (follow-up (m)) only, and cannot even do that until the rework lands. No task carries Depends_On: TASK-027, so nothing unlocks mechanically. lib/core/state stays UNFROZEN while the rework is open. Next: CX resubmits TASK-027 (4 scoped instructions); then rule (d)/(k)/(p); then dispatch TASK-028."
---
...
**Blocked_Reason:** —
**Updated_By:** ORCH
**Updated_At:** 2026-08-19T16:06:04Z
