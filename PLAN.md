---
plan_version: 1.7
last_updated: 2026-08-18T13:30:00Z
overall_status: in_progress
orchestrator_notes: "Plan v1.0 — 26 tasks from 3 specs. Territory: pubspec.yaml/analysis_options.yaml/.github/**/.gitignore/README.md FROZEN since 2c40de2. android/** serialized 001→019→026. lib/core/audio/** serialized 010→011. Dossiers in dossiers/TASK-NNN.md. S5/S5B INACTIVE — never assign. Full review/incident history in git log / REVIEW.md, not repeated here. Known unresolved risk: plan_commit doesn't re-diff before commit — real lost-update race hit twice, self-healed both times, not mechanically fixed. SPEC/PLAN FOLLOW-UPS OWED (full detail in each task's Review_Findings + REVIEW.md — index only): (a) font OFL/LICENSE missing; (b)+(c) .github/** CI pin+coverage task (after TASK-004, now landed); (d) BLOCKS TASK-007 — roomId base32 alphabet undefined, token-svc pinned RFC 4648; (e) token-svc trusts X-Forwarded-For unconditionally; (f) R4 per-room caps unbuilt; (g) keryx-evt.v1 contract vs FR-044 prose; (h) tune_burst ducking direction contradicts itself; (i) KRX-024 60ms end-of-TX marker unspecified; (j) SFX ducking release has no driver; (k) BLOCKS TASK-012/013/014 — channel/code wrap-vs-clamp undefined; (l) LINK_DEGRADED/LOCAL_FALLBACK undefined in §8.2; (m) MANDATORY BLOCKER for TASK-017+TASK-022 — lib/core/state/** is FROZEN (TASK-004 merged, 6994a11) with no owner, but RadioState lacks the flags/STN-count/active-speaker DS §6 requires (plus tuneDelta+wrap-clamp, purity-split, legacy StateNotifierProvider, naming/gating nits — full list in TASK-004 Review_Findings); ORCH must create a successor task owning this path (010→011 pattern) before TASK-017 dispatches — TASK-022 doesn't block on its OWN territory but will hit this at integration; (n) peerId (§8.6) has the same undefined base32 alphabet as roomId — TASK-009 must pin RFC 4648, must not copy TASK-006's golden peer fixture (contains a non-alphabet '9'); (o) protocol's ts/quality/seq field types are normative-by-implementation in now-frozen lib/core/protocol/** — write into §8.6 before TASK-020/021/022/024. No git remote — CI has never run. NEW SPEC uncommitted in specs/: KERYX_World_Band_Radio_Spec_v1.0.md (Phase 2, not now). REVIEWS 2026-08-18T13:12-13:19Z: TASK-004 approved on re-review (mutation test now correctly red), merged 6994a11. TASK-006 approved first-pass, merged 806bb43 — GB now 4/4 first-pass. Post-merge integrated suite 53/53 green both times. DISPATCH (2026-08-18T13:30Z): assigned TASK-022 to GB (built the TASK-006 protocol it consumes, clean track record) since it shipped Assigned_To:TBD and builders can't self-claim TBD. Dispatched GB (self-selects from {009,019,022}, transcript GB-20260818-133006.log) and CX (self-selects from {005,008}, transcript CX-20260818-133009.log) — both idle worktrees, clean dispatch. TASK-007 stays undispatched (blocked by (d)). Still owed: create the lib/core/state/** successor task per (m) before TASK-017 can ever be dispatched — not yet done. Next: /devteam-status once both builders land; then create the (m) successor task."
---
