---
plan_version: 1.3
last_updated: 2026-08-18T10:24:07Z
overall_status: in_progress
orchestrator_notes: "Plan v1.0 — first real decomposition from 3 specs (KERYX_Product_Technical_Spec_v1.1.md, KERYX_UI_Design_Specification_v1.0.md, keryx-face-prototype.html). 26 tasks. Territory rules: pubspec.yaml/analysis_options.yaml/.github/**/.gitignore/README.md are FROZEN (as of TASK-001's merge, 2c40de2). android/** is a serialized chain (001 → 019 → 026). lib/core/audio/** is a serialized chain (010 → 011). Dossiers exist for every task in dossiers/TASK-NNN.md. S5/S5B are defined but INACTIVE for this project — never assign them. Review/incident history lives in git log / REVIEW.md, not repeated here — see commits 196218e/b8171fe/c66becf/e18e820/2c40de2 if revisiting. Known unresolved risk: plan_commit doesn't re-diff immediately before its commit, so the PLAN.md lost-update race (last seen c66becf) can recur under concurrent dispatch. THREE FOLLOW-UPS OWED from TASK-001's review (none blocking, need ORCH task creation — detail in TASK-001 Review_Findings/REVIEW.md): (a) font OFL/LICENSE files missing from assets/fonts/ — must land before any release build; (b)+(c) one shared .github/** integration task for a flutter-version pin + --coverage threshold (sequence (c) after TASK-004 lands the reducer). No git remote configured — CI has never actually run, correctness is static-inspection only. NEW SPEC uncommitted in specs/: KERYX_World_Band_Radio_Spec_v1.0.md (Phase 2, not now) — needs its own /devteam-decompose pass later, do not fold into this wave. DISPATCH WAVE 2 (2026-08-18T10:24Z): TASK-001 approved+merged (2c40de2) unlocked TASK-004/005/006/007/008/009/010/019 (all Depends_On: TASK-001 only). TASK-003 (GB, token-svc/**) is needs_review, not yet reviewed — reviewed next. GB's worktree was on task/TASK-003-gb (uncommitted-relative-to-master but fully committed on that branch) — detached it back to master before this dispatch so GB's new task branch doesn't fork off TASK-003's unreviewed commits. Launched GB and CX fresh (transcripts .devteam/launch/GB-20260818-102404.log, CX-20260818-102407.log); each self-selects its highest-priority eligible task from the 8 unlocked (dispatch.ps1's builder prompt handles this, not ORCH-assigned per-task). Next: /devteam-status once builders have had time to claim; /devteam-review TASK-003."
---
