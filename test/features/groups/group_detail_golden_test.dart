...
/// TASK-091 — golden fixture for the group detail screen (V2-VT-030's
/// "detail" fixture), dark and light. Uses a real loopback
/// `FakeDirectoryServer` (same pattern as `group_detail_screen_test.dart`)
/// rather than a hand-built state, so the render exercises the real
/// `GroupDetailController.load()` path.
///
/// The server/client/controller must be built in `setUp` (real async zone),
/// not inside the `testWidgets` body — `testWidgets` runs under
/// `FakeAsync`, which cannot complete real socket I/O
/// (`FakeDirectoryServer.start`'s `HttpServer.bind`, `DirectoryClient`'s
/// real `HttpClient`).
void main() {
  ...
  Future<void> pumpAndGolden(WidgetTester tester, {required String name, required Brightness brightness}) async {
    ...
    await tester.pumpWidget(
      MaterialApp(
        ...
        home: GroupDetailScreen(controller: controller, myPk: 'me', groupSecret: const []),
      ),
    );
    // `GroupDetailScreen.initState` kicks off `controller.load()`, real
    // loopback HTTP via `DirectoryClient`/`FakeDirectoryServer` — genuine
    // `dart:io` socket I/O that `pumpAndSettle` alone cannot wait out under
    // `FakeAsync` (mirrors `new_group_screen_test.dart`'s identical fix).
    await tester.runAsync(() async {
      await Future<void>.delayed(const Duration(milliseconds: 300));
    });
    await tester.pumpAndSettle();

    await expectLater(find.byType(GroupDetailScreen), matchesGoldenFile('goldens/groups_detail_$name.png'));
  }
