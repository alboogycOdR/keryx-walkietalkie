...
void main() {
  ...
  testWidgets(
    'degraded connection reports Connection lost without rewriting '
    'configured mode (UX-FR-002; Design §5)',
    (WidgetTester tester) async {
      await pumpLanding(
        tester,
        radio: const RadioState(
  ...
  testWidgets(
    'empty memory shows the neutral message and still offers Select '
    'channel (Design §2.1)',
    (WidgetTester tester) async {
      await pumpLanding(tester);
  ...
  testWidgets(
    'watchChannel is never invoked — no 99-channel presence sweep '
    '(Design §2.1)',
    (WidgetTester tester) async {
      final LandingHarness harness = await pumpLanding(tester);
  ...
  testWidgets(
    'state cues carry icon + text; Open Talk / Select meet 48 dp '
    '(Design §3.2/§3.3; UX-FR-022)',
    (WidgetTester tester) async {
      await pumpLanding(tester);
