...
/// Hand-written [RadioHost] double for `lib/features/talk/**` widget tests —
/// same reasoning `test/app_shell/fake_radio_host.dart` documents: no real
/// transport/platform I/O can run under `flutter test`, and [RadioHost] is
/// TASK-045's own documented seam for exactly this. Records every call so a
/// test can assert the screen delegates to the host and never synthesizes a
/// result itself (Technical §5.1).
class FakeRadioHost implements RadioHost {
  ...
  /// would after e.g. a permission-denied, service-fault or engine-replaced
  /// event.
