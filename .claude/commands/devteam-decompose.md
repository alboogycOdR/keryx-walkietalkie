...
> **Model discipline:** run this command on `claude-opus-5` (the highest Opus — Fable is retired), reasoning effort **medium minimum** (high for complex or many-task waves). This is the highest-leverage judgment in the system — every builder faithfully executes whatever this step produces, so a decomposition error is the most expensive mistake in the pipeline. Effort is a depth knob, not a discount knob: territory carving and dependency sequencing are exactly where shallow reasoning misses interaction effects. Switch before proceeding; revert to your session default after.
