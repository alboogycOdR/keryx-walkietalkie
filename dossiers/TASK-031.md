...
- [2026-08-21T13:08:47Z] [GB] Rework committed `e867f8e` + `16f75af`. Host `updateRoster` is now required before size≤1 is solo proof; 50 ms delayed-roster PTT is BUSY. TX_START gets an expiring lease; PRESENCE tombstone; delayed self-GRANT ignored when a remote holder is live. Full suite 526/526, analyze clean. Scratch-merged soak 456/500 (44 remaining, all post-5s-window blind `_onGrant`/`_arbitrate` under loss+partition; crashed-holder class 0). Criterion 5 unchecked (unmodified harness still expects the defect). Criterion 6 unchecked. `test/simulation/**` not on the task branch. → needs_review.
- [2026-08-21T14:27:37Z] [GB] Resume rework #2 (ORCH 2026-08-21T15:00Z). Same branch `task/TASK-031-gb` at `82e11aa`. Residual: join-guard uses raw wall-clock since join, so a peer partitioned for the whole 5 s window self-grants on heal with no holder snapshot. Soak never calls into FloorEngine about partitions — only inbound silence is visible. Fix: pause `_inJoinGuardWindow` while no inbound for > presenceHeartbeat; restart `_observingSince` on the next inbound; stale `_idleWitnesses` do not count as idle proof while paused / are cleared on restart. `test/simulation/**` not touched.
- [2026-08-21T15:35:00Z] [GB] Resume rework #3 (ORCH 2026-08-21T16:55Z). Same branch `task/TASK-031-gb` at `6cb9bf5`. Round-3 partition-clock landed; residual reclassified: 38/45 soak failures are FR-025 emergency-preemption (out of scope, `_inJoinGuardWindow=false`); 7/45 (seeds 124/177/260/270/348/359/423) are `_onGrant` accepting a self-targeted TX_GRANT while join-guarded because the ignore only fired when `live == null` — emergency PTT skips the live-holder ignore. Narrow fix: ignore self-GRANT while join-guarded unless we are already the live holder (PRESENCE can install us before TX_GRANT). Not chasing emergency-preemption. `test/simulation/**` not touched.
- [2026-08-21T15:45:51Z] [GB] Round 4 committed `f91c3b9`. `_onGrant` now ignores a self-targeted GRANT during the join-guard when `live` is null or a remote holder; a GRANT that names us while we already hold is still accepted. Unit test covers known-holder + emergency. FR-025 tests elapsed the guard first (in-channel after the window). Scratch `wt-scratch-verify-031-r4` (TASK-031 + TASK-023-s5, not on the task branch): 500-seed soak **460/500 pass, 40 failed** (was 45). Tabulated join-guard on the NEW grantor at every flagged violation: **zero** `NEW joinGuard=true`. Original 7 in-scope seeds: 124/177/270/348/423 gone; 260/359 remain but as FR-025 (`NEW joinGuard=false`). Remaining 40 named as FR-025 emergency-preemption (join-guard already released) — criterion 6 carve-out, not chased. KNOWN ISSUE still 0/1 (A BUSY, harness still expects both TX). Timing constants 3/3. Full suite 530/530, analyze clean. `test/simulation/**` not on the task branch. → needs_review.
