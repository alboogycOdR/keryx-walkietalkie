...
def _member(client, clock) -> Agent:
    agent = Agent(client, clock, CALLSIGN)
    agent.register()
    create_group(agent, ROOM_A)
    return agent
...
def test_valid_event_token_mints(client, settings, clock) -> None:
    ...
    res = agent.request(
        "POST",
        "/token",
        {"room_id": ROOM_A, "callsign": CALLSIGN, "event_token": token},
    )
...
def test_no_expiry_event_token_accepted(client, settings, clock) -> None:
    agent = _member(client, clock)
...
def test_malformed_event_token_refused(client, clock) -> None:
    agent = _member(client, clock)
    res = agent.request(
        "POST",
        "/token",
        {"room_id": ROOM_A, "callsign": CALLSIGN, "event_token": "not-a-token"},
    )
    assert res.status_code == 403
    assert res.json()["detail"] == "invalid_event_token"
