...
- **Current focus (updated 2026-09-14):** REAL-DEVICE ACCEPTANCE of the v2 KERYX-ID/directory pivot against the live relay — the code-side work is done and mutation-tested, but the actual two-phone field test still fails.
  ...
  - **Presence never actually connected in the field until 2026-09-14 (TASK-109/110/111, merged `854f975`) — found during the owner's first real two-phone retest, not by any test in this repo.** Root cause, confirmed empirically: `directoryBaseUri()` hard-forces `https`, and `WebSocket.connect` throws `Unsupported URL scheme 'https'` before any network I/O — so the presence WebSocket never opened, on either side (the deployed token-svc Docker image also shipped with no `websockets` dependency at all, despite registering a `/presence` WS route). Fixed: `PresenceClient` now maps `https→wss`/`http→ws` and `token-svc` pins `websockets`; a device that registers but never presses PTT now announces `available` on connect (TASK-109) instead of staying `offline` by default; Talk/PTT now seeds a contact's presence from the already-loaded Contacts snapshot instead of showing Offline until the next live update arrives (TASK-111, since the relay sends no roster snapshot on connect). This very plausibly explains the "Couldn't send that request." contact-add symptom recorded in the previous version of this note — worth re-testing on real devices before chasing anything else.
  - **Next:**
    1. Re-test contact-add and presence end-to-end on the owner's two real phones now that the WebSocket fix has landed — this has never been proven on real hardware.
    2. TASK-112 (filed, unassigned): `journey_two_phones_test.dart`'s receive-side gate is intermittently flaky (resolves `direct` transport instead of `relay` under specific test-ordering) — root-cause before leaning on it further as the acceptance proxy.
    3. TASK-059's remaining rows (contention, channel change, background, no-internet LAN, network drop) and TASK-060 (LINKED), both needing the owner's two phones.
    4. TASK-061 (retire the legacy face), then TASK-062 (release acceptance).
  - **Environment notes:**
